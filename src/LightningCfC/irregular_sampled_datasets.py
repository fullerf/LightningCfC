...
class Walker2dImitationData:
    def __init__(self, seq_len):
        ...
        all_files = sorted(
            [str(f) for f in resources.files('LightningCfC.data.walker').glob('*.npy')]
        )
        assert len(all_files) > 0, "could not fild walker files"
...
class ETSMnistData:
    def __init__(self, time_major=False, pad_size=256):
        self.threshold = 128
    ...
    def load_from_cache(self):
        try:
            cache_dir = resources.files('data.MNIST.dataset')
        except ModuleNotFoundError:
            return False
        self.train_events = np.load(cache_dir / "train_events.npy")
        self.train_elapsed = np.load(cache_dir / "train_elapsed.npy")
        self.train_mask = np.load(cache_dir / "train_mask.npy")
        self.train_y = np.load(cache_dir / "train_y.npy")

        self.test_events = np.load(cache_dir / "test_events.npy")
        self.test_elapsed = np.load(cache_dir / "test_elapsed.npy")
        self.test_mask = np.load(cache_dir / "test_mask.npy")
        self.test_y = np.load(cache_dir / "test_y.npy")
    ...
    def transform_sample(self, x):
        ...
        mask = np.zeros([self.pad_size], dtype=bool)
    ...
    def create_dataset(self):
        ds = torchvision.datasets.MNIST(root=resources.files('data'), download=True)
        train_x = ds.data.clone().numpy()
        train_y = ds.targets.clone().numpy()
        ds = torchvision.datasets.MNIST(root=resources.files('data'), download=True, train=False)
        test_x = ds.data.clone().numpy()
        test_y = ds.targets.clone().numpy()
        (resources.files('data') / 'MNIST' / '__init__.py').touch()
        ...
        # note data.MNIST is created by torchvision if it does not exist
        write_dir = resources.files('data.MNIST') / 'dataset'
        write_dir.mkdir(parents=True, exist_ok=True)
        (write_dir / '__init__.py').touch()
        np.save(write_dir / "train_events.npy", self.train_events)
        np.save(write_dir / "train_elapsed.npy", self.train_elapsed)
        np.save(write_dir / "train_mask.npy", self.train_mask)
        np.save(write_dir / "train_y.npy", self.train_y)

        np.save(write_dir / "test_events.npy", self.test_events)
        np.save(write_dir / "test_elapsed.npy", self.test_elapsed)
        np.save(write_dir / "test_mask.npy", self.test_mask)
        np.save(write_dir / "test_y.npy", self.test_y)
...
class PersonData:
    ...
    def load_crappy_formated_csv(self):
        ...
        last_millis = None
        maybe_data = [str(f) for f in resources.files('LightningCfC.data.person').glob('*.txt')]
        assert len(maybe_data)>0, "cannot find data for person dataset"
        data_file = maybe_data[0]
...
class XORData:
    def __init__(self, time_major=False, event_based=True, pad_size=24):
        self.pad_size = pad_size
    ...
    def load_from_cache(self):
        try:
            data_dir = resources.files('data.xor_dataset')
            if len([str(f) for f in data_dir.glob('*.npy')]) == 0:
                return False
        except ModuleNotFoundError:
            return False
        self.train_events = np.load(data_dir / "xor_train_events.npy")
        self.train_elapsed = np.load(data_dir / "xor_train_elapsed.npy")
        self.train_mask = np.load(data_dir / "xor_train_mask.npy")
        self.train_y = np.load(data_dir / "xor_train_y.npy")

        self.test_events = np.load(data_dir / "xor_test_events.npy")
        self.test_elapsed = np.load(data_dir / "xor_test_elapsed.npy")
        self.test_mask = np.load(data_dir / "xor_test_mask.npy")
        self.test_y = np.load(data_dir / "xor_test_y.npy")
    ...
    def create_dataset(self):
        ...
        print("Abort counter: ", str(self._abort_counter))
        parent = resources.files('data')
        write_dir = parent / "xor_dataset"
        os.makedirs(str(write_dir), exist_ok=True)
        (write_dir / '__init__.py').touch()
        np.save(write_dir / "xor_train_events.npy", self.train_events)
        np.save(write_dir / "xor_train_elapsed.npy", self.train_elapsed)
        np.save(write_dir / "xor_train_mask.npy", self.train_mask)
        np.save(write_dir / "xor_train_y.npy", self.train_y)

        np.save(write_dir / "xor_test_events.npy", self.test_events)
        np.save(write_dir / "xor_test_elapsed.npy", self.test_elapsed)
        np.save(write_dir / "xor_test_mask.npy", self.test_mask)
        np.save(write_dir / "xor_test_y.npy", self.test_y)
